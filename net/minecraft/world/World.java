...
import net.xtrafrancyz.LightUpdateGuard;
import net.xtrafrancyz.mods.texteria.PlayerCollisionTickHandler;
...
public abstract class World implements IBlockAccess
{
    ...
    public LightUpdateGuard lightGuard = new LightUpdateGuard();
    ...
    {
        return p_175701_0_.getX() >= -30000000 && p_175701_0_.getZ() >= -30000000 && p_175701_0_.getX() < 30000000 && p_175701_0_.getZ() < 30000000 && p_175701_0_.getY() >= 0 && p_175701_0_.getY() < 256;
    }
    ...
    {
        return isValid(pos) && this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, allowEmpty);
    }
    ...
    public int getLightFor(EnumSkyBlock type, BlockPos pos)
    {
        ...
        return pos.getY() < 256 && this.chunkProvider.chunkExists(pos.getX() >> 4, pos.getZ() >> 4) ? this.chunkProvider.provideChunk(pos.getX() >> 4, pos.getZ() >> 4).getLightFor(type, pos) : type.defaultLightValue;
    }
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb)
    {
        ...
        {
            Entity entity = (Entity)list1.get(j2);
    ...
    public void updateEntities()
    {
        ...
        this.lightGuard.tick();
        PlayerCollisionTickHandler.tick();
    }
    ...
    private int getRawLight(BlockPos pos, EnumSkyBlock lightType)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    blockpos$mutableblockpos.func_181079_c(pos.getX() + enumfacing.getFrontOffsetX(), pos.getY() + enumfacing.getFrontOffsetY(), pos.getZ() + enumfacing.getFrontOffsetZ());
                    int k = this.getLightFor(lightType, blockpos$mutableblockpos) - j;
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos)
    {
        ...
        else if (!this.lightGuard.startUpdate(pos))
        {
            return false;
        }
        else
        {
            ...
            else
            {
                ...
                int l = this.getLightFor(lightType, pos);
                int i1 = this.getRawLight(pos, lightType);
                int j1 = pos.getX();
                int k1 = pos.getY();
                int l1 = pos.getZ();
                BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
                ...
                if (i1 > l)
                {
                    this.lightUpdateBlockList[k++] = 133152;
                }
                else if (i1 < l)
                {
                    this.lightUpdateBlockList[k++] = 133152 | l << 18;

                    while (j < k)
                    {
                        ++i;
                        int i2 = this.lightUpdateBlockList[j++];
                        int j2 = (i2 & 63) - 32 + j1;
                        int k2 = (i2 >> 6 & 63) - 32 + k1;
                        int l2 = (i2 >> 12 & 63) - 32 + l1;
                        int i3 = i2 >> 18 & 15;
                        blockpos$mutableblockpos.func_181079_c(j2, k2, l2);
                        int j3 = this.getLightFor(lightType, blockpos$mutableblockpos);

                        if (j3 == i3)
                        {
                            this.setLightFor(lightType, blockpos$mutableblockpos, 0);

                            if (i3 > 0)
                            {
                                int k3 = MathHelper.abs_int(j2 - j1);
                                int l3 = MathHelper.abs_int(k2 - k1);
                                int i4 = MathHelper.abs_int(l2 - l1);
                                ...
                                if (k3 + l3 + i4 < 17)
                                {
                                    ...
                                    for (EnumFacing enumfacing : EnumFacing.VALUES)
                                    {
                                        int j4 = j2 + enumfacing.getFrontOffsetX();
                                        int k4 = k2 + enumfacing.getFrontOffsetY();
                                        int l4 = l2 + enumfacing.getFrontOffsetZ();
                                        blockpos$mutableblockpos1.func_181079_c(j4, k4, l4);
                                        int i5 = Math.max(1, this.getBlockState(blockpos$mutableblockpos1).getBlock().getLightOpacity());
                                        j3 = this.getLightFor(lightType, blockpos$mutableblockpos1);

                                        if (j3 == i3 - i5 && k < this.lightUpdateBlockList.length)
                                        {
                                            this.lightUpdateBlockList[k++] = j4 - j1 + 32 | k4 - k1 + 32 << 6 | l4 - l1 + 32 << 12 | i3 - i5 << 18;
                                        }
                                    }
                                }
                            }
                ...
                {
                    ++i;
                    int j5 = this.lightUpdateBlockList[j++];
                    int k5 = (j5 & 63) - 32 + j1;
                    int l5 = (j5 >> 6 & 63) - 32 + k1;
                    int i6 = (j5 >> 12 & 63) - 32 + l1;
                    BlockPos blockpos = blockpos$mutableblockpos.func_181079_c(k5, l5, i6);
                    int j6 = this.getLightFor(lightType, blockpos);
                    int k6 = this.getRawLight(blockpos, lightType);

                    if (k6 != j6)
                    {
                        this.setLightFor(lightType, blockpos, k6);

                        if (k6 > j6)
                        {
                            int l6 = Math.abs(k5 - j1);
                            int i7 = Math.abs(l5 - k1);
                            int j7 = Math.abs(i6 - l1);
                            boolean flag = k < this.lightUpdateBlockList.length - 6;

                            if (l6 + i7 + j7 < 17 && flag)
                            {
                                if (this.getLightFor(lightType, blockpos.west()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 - 1 - j1 + 32 + (l5 - k1 + 32 << 6) + (i6 - l1 + 32 << 12);
                                }

                                if (this.getLightFor(lightType, blockpos.east()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 + 1 - j1 + 32 + (l5 - k1 + 32 << 6) + (i6 - l1 + 32 << 12);
                                }

                                if (this.getLightFor(lightType, blockpos.down()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 - j1 + 32 + (l5 - 1 - k1 + 32 << 6) + (i6 - l1 + 32 << 12);
                                }

                                if (this.getLightFor(lightType, blockpos.up()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 - j1 + 32 + (l5 + 1 - k1 + 32 << 6) + (i6 - l1 + 32 << 12);
                                }

                                if (this.getLightFor(lightType, blockpos.north()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 - j1 + 32 + (l5 - k1 + 32 << 6) + (i6 - 1 - l1 + 32 << 12);
                                }

                                if (this.getLightFor(lightType, blockpos.south()) < k6)
                                {
                                    this.lightUpdateBlockList[k++] = k5 - j1 + 32 + (l5 - k1 + 32 << 6) + (i6 + 1 - l1 + 32 << 12);
                                }
                            }
