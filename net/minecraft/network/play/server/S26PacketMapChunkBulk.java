...
public class S26PacketMapChunkBulk implements Packet<INetHandlerPlayClient>
{
    ...
    {
        if (!this.decrypted && S21PacketChunkData.canDecrypt() && Brainstorm.isChunksObfuscated())
        {
            this.decrypted = true;
            int[] aint = new int[this.xPositions.length + this.zPositions.length];
            System.arraycopy(this.xPositions, 0, aint, 0, this.xPositions.length);
            System.arraycopy(this.zPositions, 0, aint, this.xPositions.length, this.zPositions.length);
            byte[] abyte = S21PacketChunkData.intsToBytes(aint);
            ...
            aint = S21PacketChunkData.bytesToInts(abyte);
            System.arraycopy(aint, 0, this.xPositions, 0, this.xPositions.length);
            System.arraycopy(aint, this.xPositions.length, this.zPositions, 0, this.zPositions.length);
        }
