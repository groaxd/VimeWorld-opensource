...
{
    private static final ObfValue.OFloat OBFVAL_lKz5h = ObfValue.create(0.5F);
    private static final ObfValue.ODouble OBFVAL_UoL8V = ObfValue.create(9.0D);
    private static final ObfValue.OFloat OBFVAL_X3v2I = ObfValue.create(10.0F);
    private static final ObfValue.OFloat OBFVAL_ftEbM = ObfValue.create(4.0F);
    private static final ObfValue.OInteger OBFVAL_GJnfD = ObfValue.create(2001);
    private static final ObfValue.OFloat OBFVAL_9bT5f = ObfValue.create(8.0F);
    private static final ObfValue.OFloat OBFVAL_0jr1R = ObfValue.create(-180.0F);
    private static final ObfValue.OFloat OBFVAL_w1FM8 = ObfValue.create(4.5F);
    private static final ObfValue.OInteger OBFVAL_Psrht = ObfValue.create(5);
    private static final ObfValue.OFloat OBFVAL_31H62 = ObfValue.create(5.0F);
    private static final ObfValue.ODouble OBFVAL_JIBk3 = ObfValue.create(-1.0D);
    ...
    private final List<Packet> endOfTickQueue = new ArrayList();
    private boolean justStartBreakingBlock = false;
    ...
    {
        playerIn.rotationYaw = OBFVAL_0jr1R.get();
    }
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos, EnumFacing side)
    {
        ...
        else
        {
            ...
            {
                world.playAuxSFX(OBFVAL_GJnfD.get(), pos, Block.getStateId(iblockstate));
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face)
    {
        ...
        else
        {
            if (this.currentGameType.isCreative())
            {
                ...
                this.blockHitDelay = OBFVAL_Psrht.get();
            }
            else if (!this.isHittingBlock || !this.isHittingPosition(loc))
            {
                ...
                if (flag)
                {
                    f = block1.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, loc);
                }

                Packet packet = new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.START_DESTROY_BLOCK, loc, face);

                if (this.mc.thePlayer.onGround && !this.mc.thePlayer.serverOnGroundState && f >= 1.0F)
                {
                    this.endOfTickQueue.add(packet);
                }
                else
                {
                ...
                {
                    this.justStartBreakingBlock = true;
                    this.isHittingBlock = true;
                    this.currentBlock = loc;
                    this.currentItemHittingBlock = this.mc.thePlayer.getHeldItem();
                    this.curBlockDamageMP = 0.0F;
                    this.stepSoundTickCounter = 0.0F;
                    this.mc.theWorld.sendBlockBreakProgress(this.mc.thePlayer.getEntityId(), this.currentBlock, (int)(this.curBlockDamageMP * OBFVAL_X3v2I.get()) - 1);
                }
            }
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing)
    {
        ...
        else if (this.isHittingPosition(posBlock))
        {
            ...
            else
            {
                ...
                if (this.stepSoundTickCounter % OBFVAL_ftEbM.get() == 0.0F)
                {
                    this.mc.getSoundHandler().playSound(new PositionedSoundRecord(new ResourceLocation(block.stepSound.getStepSound()), (block.stepSound.getVolume() + 1.0F) / OBFVAL_9bT5f.get(), block.stepSound.getFrequency() * OBFVAL_lKz5h.get(), (float)posBlock.getX() + OBFVAL_lKz5h.get(), (float)posBlock.getY() + OBFVAL_lKz5h.get(), (float)posBlock.getZ() + OBFVAL_lKz5h.get()));
                }
        ...
        {
            this.justStartBreakingBlock = false;
            boolean flag = this.clickBlock(posBlock, directionFacing);

            if (this.justStartBreakingBlock)
            {
                this.onPlayerDamageBlock(posBlock, directionFacing);
            }
    ...
    {
        return this.currentGameType.isCreative() ? OBFVAL_31H62.get() : OBFVAL_w1FM8.get();
    }
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn)
    {
        ...
        else
        {
            ...
            Packet packet = new C08PacketPlayerBlockPlacement(playerIn.inventory.getCurrentItem());

            if ((itemStackIn.getItem() == Items.lava_bucket || itemStackIn.getItem() == Items.water_bucket) && itemstack != null && itemstack.getItem() == Items.bucket && playerIn.motionY > OBFVAL_JIBk3.get())
            {
                this.endOfTickQueue.add(packet);
            }
            else
    ...
    {
        if (this.mc.entityRenderer.hitDistance <= OBFVAL_UoL8V.get() || this.isInCreativeMode())
        {
            this.syncCurrentPlayItem();
    ...
    {
        for (Packet packet : this.endOfTickQueue)
        {
            this.netClientHandler.addToSendQueue(packet);
        }

        this.endOfTickQueue.clear();
    }

    static
    {
        ObfValue.beginGeneration();
        ObfValue.endGeneration(new byte[] {(byte)89, (byte) - 102, (byte)56, (byte) - 23, (byte) - 81, (byte)45, (byte) - 30, (byte)86});
    }
}
