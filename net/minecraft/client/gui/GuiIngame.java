...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (!TexteriaOptions.disableTab && this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null))
        {
        ...
        Texteria.instance.gui.render(GuiRenderLayer.HUD);
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_)
    {
        ...
        if (i > 0)
        {
            ...
            int k = p_175176_1_.getScaledHeight() - 32 + 3;
            this.drawTexturedModalRect(p_175176_2_, k, 0, 64, short1, 5);

            if (j > 0)
            {
                this.drawTexturedModalRect(p_175176_2_, k, 0, 69, j, 5);
            }
        }
        ...
        if (this.mc.thePlayer.experienceLevel > 0)
        {
            ...
            int j1 = (p_175176_1_.getScaledWidth() - this.getFontRenderer().getStringWidth(s)) / 2;
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        {
            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(((Score)object).getPlayerName());
            String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, ((Score)object).getPlayerName()) + ": " + EnumChatFormatting.RED + ((Score)object).getScorePoints();
        ...
        {
            ++j;
            ScorePlayerTeam scoreplayerteam1 = scoreboard.getPlayersTeam(((Score)object1).getPlayerName());
            String s1 = ScorePlayerTeam.formatPlayerName(scoreplayerteam1, ((Score)object1).getPlayerName());
            String s2 = EnumChatFormatting.RED + "" + ((Score)object1).getScorePoints();
        ...
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            if (entity == null)
            {
                ...
                for (int k5 = 0; k5 < 10; ++k5)
                {
                    ...
                    int i8 = j1 - k5 * 8 - 9;
                    this.drawTexturedModalRect(i8, j6, 16 + b4 * 9, 27, 9, 9);

                    if (flag1)
                    {
                        if (k5 * 2 + 1 < l)
                        {
                            this.drawTexturedModalRect(i8, j6, l6 + 54, 27, 9, 9);
                        }

                        if (k5 * 2 + 1 == l)
                        {
                            this.drawTexturedModalRect(i8, j6, l6 + 63, 27, 9, 9);
                        }
                    }

                    if (k5 * 2 + 1 < k)
                    {
                        this.drawTexturedModalRect(i8, j6, l6 + 36, 27, 9, 9);
                    }

                    if (k5 * 2 + 1 == k)
                    {
                        this.drawTexturedModalRect(i8, j6, l6 + 45, 27, 9, 9);
                    }
                }
            }
            else if (entity instanceof EntityLivingBase)
            {
                ...
                int i6 = (int)Math.ceil((double)entitylivingbase.getHealth());
                ...
                for (int j8 = 0; j7 > 0; j8 += 20)
                {
                    ...
                    for (int k4 = 0; k4 < j4; ++k4)
                    {
                        ...
                        int l4 = j1 - k4 * 8 - 9;
                        this.drawTexturedModalRect(l4, l7, b2 + b3 * 9, 9, 9, 9);

                        if (k4 * 2 + 1 + j8 < i6)
                        {
                            this.drawTexturedModalRect(l4, l7, b2 + 36, 9, 9, 9);
                        }

                        if (k4 * 2 + 1 + j8 == i6)
                        {
                            this.drawTexturedModalRect(l4, l7, b2 + 45, 9, 9, 9);
                        }
                    }
    ...
    private void renderHotbarItem(int index, int xPos, int yPos, float partialTicks, EntityPlayer p_175184_5_)
    {
        ...
        if (itemstack != null)
        {
            ...
            {
                int i = nbttagcompound.getInteger("SlotColor");

                if (i != 0)
                {
                    GlStateManager.disableLighting();
                    GlStateManager.disableDepth();
                    drawRect(xPos, yPos, xPos + 16, yPos + 16, formatSlotColor(i, true));
                    GlStateManager.enableLighting();
                    GlStateManager.enableDepth();
                }
                else
                {
                    int j = nbttagcompound.getInteger("SlotOutlineColor");

                    if (j != 0)
                    {
                        GlStateManager.disableLighting();
                        GlStateManager.disableDepth();
                        this.drawHollowSquare(xPos, yPos, 16, formatSlotColor(j, true));
                        GlStateManager.enableLighting();
                        GlStateManager.enableDepth();
                    }
                }
    ...
    public static int formatSlotColor(int p_formatSlotColor_0_, boolean p_formatSlotColor_1_)
    {
        if (p_formatSlotColor_1_)
        {
            int i = p_formatSlotColor_0_ >> 24 & 255;
            i = (int)((double)i * 0.7D);
            p_formatSlotColor_0_ = (p_formatSlotColor_0_ & 16777215) + (i << 24);
        }

        return p_formatSlotColor_0_;
    }
