...
import net.xtrafrancyz.mods.texteria.gui.TexteriaTooltip;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    public void drawHoveringText(List<String> textLines, int x, int y)
    {
        if (!textLines.isEmpty())
        {
            ...
            int i = 0;
            Map<Integer, GuiElementWrapper> map = TexteriaTooltip.tempMap;
            map.clear();

            for (int j = 0; j < textLines.size(); ++j)
            {
                String s = (String)textLines.get(j);
                GuiElementWrapper guielementwrapper = TexteriaTooltip.getTexteriaLine(s);
                int k;

                if (guielementwrapper != null)
                {
                    if (guielementwrapper.isVisible(this.mc))
                    {
                        k = MathHelper.ceiling_float_int(guielementwrapper.element.getScaledWidth() + guielementwrapper.element.getXY(Texteria.time).x);
                    }
                    else
                    {
                        k = 0;
                    ...
                    map.put(Integer.valueOf(j), guielementwrapper);
                }
                else
                {
            ...
            if (map.isEmpty())
            {
                ...
                if (textLines.size() > 1)
                {
                    k1 += 2 + (textLines.size() - 1) * 10;
                }
            }
            else
            {
                int l1 = textLines.size() - map.size();
                ...
                for (GuiElementWrapper guielementwrapper1 : map.values())
                {
                    if (guielementwrapper1.isVisible(this.mc))
                    {
                        k1 = (int)((float)k1 + guielementwrapper1.element.getScaledHeight() + guielementwrapper1.element.getXY(Texteria.time).y);
                    }
                }
            }
            ...
            int i2 = -267386864;
            this.drawGradientRect(i1 - 3, j1 - 4, i1 + i + 3, j1 - 3, i2, i2);
            this.drawGradientRect(i1 - 3, j1 + k1 + 3, i1 + i + 3, j1 + k1 + 4, i2, i2);
            this.drawGradientRect(i1 - 3, j1 - 3, i1 + i + 3, j1 + k1 + 3, i2, i2);
            this.drawGradientRect(i1 - 4, j1 - 3, i1 - 3, j1 + k1 + 3, i2, i2);
            this.drawGradientRect(i1 + i + 3, j1 - 3, i1 + i + 4, j1 + k1 + 3, i2, i2);
            int j2 = 1347420415;
            int k2 = (j2 & 16711422) >> 1 | j2 & -16777216;
            this.drawGradientRect(i1 - 3, j1 - 3 + 1, i1 - 3 + 1, j1 + k1 + 3 - 1, j2, k2);
            this.drawGradientRect(i1 + i + 2, j1 - 3 + 1, i1 + i + 3, j1 + k1 + 3 - 1, j2, k2);
            this.drawGradientRect(i1 - 3, j1 - 3, i1 + i + 3, j1 - 3 + 1, j2, j2);
            this.drawGradientRect(i1 - 3, j1 + k1 + 2, i1 + i + 3, j1 + k1 + 3, k2, k2);

            for (int l = 0; l < textLines.size(); ++l)
            {
                String s1 = (String)textLines.get(l);
                GuiElementWrapper guielementwrapper2 = TexteriaTooltip.getTexteriaLine(s1);

                if (guielementwrapper2 != null)
                {
                    if (guielementwrapper2.isVisible(this.mc))
                    {
                        GlStateManager.translate((float)i1, (float)j1 - 0.5F, 0.0F);
                        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                        guielementwrapper2.element.renderInContainer(Texteria.time);
                        GlStateManager.translate((float)(-i1), (float)(-j1) + 0.5F, 0.0F);
                        j1 = (int)((float)j1 + guielementwrapper2.element.getScaledHeight() + guielementwrapper2.element.getXY(Texteria.time).y);
                    }
                }
                else
                {
                    this.fontRendererObj.drawStringWithShadow(s1, (float)i1, (float)j1, -1);
                    j1 += 10;
                }
