    ...
    private void drawSlot(Slot slotIn)
    {
        ...
        if (!flag1)
        {
            if (itemstack != null && itemstack.hasTagCompound())
            {
                ...
                int l = nbttagcompound.getInteger("SlotColor");

                if (l != 0)
                {
                    GlStateManager.disableLighting();
                    GlStateManager.disableDepth();
                    drawRect(i, j, i + 16, j + 16, GuiIngame.formatSlotColor(l, false));
                    GlStateManager.enableLighting();
                    GlStateManager.enableDepth();
                }
                else
                {
                    int k = nbttagcompound.getInteger("SlotOutlineColor");

                    if (k != 0)
                    {
                        GlStateManager.disableLighting();
                        GlStateManager.disableDepth();
                        this.drawHollowSquare(i, j, 16, GuiIngame.formatSlotColor(k, false));
                        GlStateManager.enableLighting();
                        GlStateManager.enableDepth();
                    }
                }
